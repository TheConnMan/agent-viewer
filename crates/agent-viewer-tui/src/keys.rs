...
pub(crate) mod tests {
    use super::{
        ensure_completions, handle_attached_key, handle_paste, handle_rename_key, is_quit_chord,
        open_filter,
    };
    use crate::{NoticeState, Ui};
    use agent_viewer_core::pty::{PtySession, PtySpec};
    use agent_viewer_core::{BackendKind, Session, Status};
    use agent_viewer_tui::app::{App, Composer, DetachTracker};
    ...
    pub(crate) fn test_ui_with(sessions: Vec<Session>) -> Ui {
        Ui {
            ...
            mutation_executor: std::sync::Arc::new(|_| Ok(String::new())),
