...
pub struct Theme {
    ...
    pub ok: Color,
    pub warn: Color,
    /// The attention token: reserved for "this one is waiting on you". Deliberately a hue no
    /// other token owns, because `warn` sits a few points from `accent` in most themes and a
    /// blocked tile was indistinguishable from a focused one.
    pub attn: Color,
    pub err: Color,
    pub merged: Color,
    pub cc: Color,
