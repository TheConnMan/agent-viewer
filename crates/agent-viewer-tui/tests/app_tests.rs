...
fn toggle_hidden_includes_hidden_rows() {
    ...
    assert!(
        session_rows(&rows)
            .iter()
            .all(|r| !matches!(r, Row::Session { hidden: true, .. }))
    );
    ...
    assert!(
        rows.iter()
            .any(|r| matches!(r, Row::Session { hidden: true, id, .. } if id == "hid"))
    );
...
fn collapse_emits_header_only_with_count() {
    ...
    assert!(
        !rows
            .iter()
            .any(|r| matches!(r, Row::Session { id, .. } if id == "a1" || id == "a2"))
    );

    // Group B still shows its session.
    assert!(
        rows.iter()
            .any(|r| matches!(r, Row::Session { id, .. } if id == "b1"))
    );
}
