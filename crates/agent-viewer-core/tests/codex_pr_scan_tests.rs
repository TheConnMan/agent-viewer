...
// `rollout-2026-08-10T18-57-32` (ExampleOrg/case-management#436), trimmed only in the
...
fn scanner_reports_pr_from_the_live_plaintext_result_shape() {
    ...
    append(
        ...
        &custom_tool_call(
            ...
            "exec",
            r#"const r = await tools.exec_command({"cmd":"git commit --no-verify -m \"Use mail icon\" && git push -u origin HEAD && gh pr create --base develop --title \"Use mail icon\"","workdir":"/home/example/git/example-org/case-management"});"#,
        ),
    );
    append(
        &path,
    ...
    assert_eq!(
        hrefs(&scanner.refs_for(&path, &mut budget)),
        vec!["https://github.com/ExampleOrg/case-management/pull/436".to_string()]
    );
}
        ...
        serde_json::json!({
            ...
            "payload": {
                ...
                "invocation": {
                    "server": "codex_apps",
                    "tool": tool,
                    "arguments": {"repository_full_name": "ExampleOrg/case-management"},
                },
                "app_name": "GitHub",
...
// The second creation channel, and the one that opened ExampleOrg/case-management#437:
...
fn scanner_reports_pr_opened_through_the_github_connector() {
    ...
    append(
        ...
        &connector_pr_create(
            ...
            false,
            "https://github.com/ExampleOrg/case-management/pull/437",
        ),
    );
    ...
    assert_eq!(
        hrefs(&scanner.refs_for(&path, &mut budget)),
        vec!["https://github.com/ExampleOrg/case-management/pull/437".to_string()]
    );
}
