...
`PATH`, on both `auto` and concrete providers. The router names the job and records the
decision, so a job started from the viewer is tracked the same way as one dispatched from
anywhere else.
...
- `Ctrl+K` — command palette. It carries every action in this list (each with its chord, and
  ...
  every discovered model for each backend, and the same owned command entries as the completion
  popup. Selecting one inserts its exact `/name ` or `$name ` text and pins its provider.
